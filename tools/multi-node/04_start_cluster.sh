#!/bin/bash
# openGauss 多节点集群 - 启动脚本（增强版）
set -e

echo "[1/4] 发现并启动停止的容器"

# 检查主节点容器
if docker ps -a -q -f name=opengauss-primary | grep -q .; then
    if ! docker ps -q -f name=opengauss-primary | grep -q .; then
        echo "  主节点容器已停止，正在启动容器..."
        docker start opengauss-primary > /dev/null
        sleep 3
        echo "  ✓ 主节点容器已启动"
    else
        echo "  ✓ 主节点容器运行中"
...
STANDBY_CONTAINERS=()
for i in {1..10}; do
    if docker ps -a -q -f name=opengauss-standby$i | grep -q .; then
        STANDBY_CONTAINERS+=("opengauss-standby$i")
        if ! docker ps -q -f name=opengauss-standby$i | grep -q .; then
            echo "  备节点容器 opengauss-standby$i 已停止，正在启动容器..."
            docker start opengauss-standby$i > /dev/null
            sleep 2
            echo "  ✓ opengauss-standby$i 容器已启动"
        fi
    fi
done
...
if [ $STANDBY_COUNT -gt 0 ]; then
    echo "[3/4] 启动备节点数据库 (${STANDBY_COUNT}个)"
    ...
    echo "[3/4] 跳过备节点启动 (无备节点)"
fi

echo "[4/4] 验证集群状态"
sleep 2
docker exec opengauss-primary su - omm -c "gs_ctl query -D /home/omm/data" | grep -E "(db_state|static_connections)" || true
